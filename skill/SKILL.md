...
7. Run the acceptance command yourself, `| tail -30`. It has to actually run — reading the diff is not verification. Red → one more round; red again → `board task blocked $T --note "<what failed>" --cmd "<the acceptance command>"` → back to 1. The `--cmd` is what lets the human reproduce the red on their own machine without reading the note for it.
8. `board task pr $T` — pushes the branch and opens the PR through the manifest's forge.
9. Review: 3 FRESH `model: sonnet` agents. `prompts/reviewer.md` lives in this skill's own directory — resolve its absolute path the same way as step 6 and pass it explicitly, prompt: "Read <resolved skill dir>/prompts/reviewer.md. PR: <url>. Worktree: <path>. Register under your OWN BOARD_SESSION and BOARD_CACHE (see reviewer.md) and run `board task review $T --open N --fixed M` yourself — you do not run it." NEVER `fork` — a fork inherits your merge mandate. risk=high ∧ PHASE ∈ {launch, live} → add one `model: opus`. Log each with `--dispatch reviewer:<model> --tokens <N>` from that agent's usage block. The gate rejects a review result set by the owner, and you ARE the owner — a subagent inherits your session, so reporting it yourself always fails the gate. The gate needs `--open 0` from every reviewer in the round. Findings ≥80 → fix via a subagent, then `board task review $T --fixed M --round` (starts a new round, drops old results) and re-review, max 2 rounds; still open → `board task blocked $T` → 1.
...
- Product choice → `board ask --task $T --default "<best guess>" --deadline 8h "<q>"`, implement the default, mark the PR "assumes X (Q-n)". risk=high never merges on a default. **Always `--task $T`** — that is what puts the question, with a link to it, on `board.<host>/t/$T`, where the human actually looks.
- Only a human can do it → `board task blocked $T --note "needs human: …"` → next task.
- **Anything the human is meant to RUN goes on the card with `--cmd`, ready to paste**: `board ask --cmd "<command>"`, `board task blocked $T --cmd "<command>"`, `board task progress $T --cmd "<command>"`. The flag may be repeated, and the command lands on `/t/$T` with a copy button. Never write the command in prose only and expect it to be typed back in off a phone.
