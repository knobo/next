...
7. Run the acceptance command yourself, `| tail -30`. It has to RUN — a CLI invocation, a playwright run, test code. Reading the diff is not verification and neither is "looks right". Red → one more round; red again → `board task blocked $T --note "<what failed>"` → back to 1.
...
10. Prove the SURFACE works, not only the unit (`reference/testing.md`): a UI change gets a playwright run, an endpoint a real request, a script an execution. Visual changes are always playwright — never "a human should look at this". Cannot reach the surface at all (no `playwright`, no env, nothing to run it against)? That is a finding — `board ask` or `board task blocked $T` — not a card asking the human to check it for you.
...
14. It is live — say in ONE line what the human should look at and where: `board task progress $T
    "dev: <url> — <what is new>"`. They test what is deployed, after the fact, and it blocks
    nothing. Then `board task cleanup $T` (refuses unless merged) and `board task done $T`.
