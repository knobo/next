...
| Review result | `board task review T-42 --open 0 --fixed 2` | `POST /tasks/T-42/review` | Gate input. The gate refuses a review set by the task's own owner. |
...
| Human test | `board test request T-42 --card-file card.json` | `POST /questions` `{kind:"test", card:{…}}` | Schema check; pushes the test card (§5); status `awaiting_human` when the phase requires a human OK before merge. `board test-level T-42` gives `auto\|human` + `env` from phase × change class. |
| Test queue | `board tests [--project X]` (the human) / `/tests` (HTML) | `GET /questions?kind=test&status=open` | One place across all projects (§5b). Answer with `board answer <id> ok` / `fail --note "…"`, or the buttons on the page. |
