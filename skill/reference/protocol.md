...
| Create task | `board task create --repo web --title ... --spec-file t.md --requires browser-test --risk high --touches 'src/routes/**'` | `POST /tasks` | Any agent may do this (a finding → a new task for somebody else). `spec` is rendered as markdown on `/t/<id>` — headings, lists, code, quotes and http links — so write it for a reader. |
...
| Estimate | `board task estimate T-42 5` (or `--estimate 5` on create) | `PATCH /tasks/T-42` | A RELATIVE size: one of 1, 2, 3, 5, 8, 13, and nothing else — the number is only worth writing down if it means the same thing on every task and from every agent. The board records which agent and which **model** gave it, so it can answer how well a model sizes its own work. Anything off the scale is refused (400) with the scale in the body. |
| Reprioritise / resize | `board task patch T-42 --priority 80` | `PATCH /tasks/T-42` | `priority` and `estimate` describe the QUEUE and need only project membership — they are set while grooming, before anyone has claimed anything. `repo`, `touches` and `risk` describe the WORK and stay the owner's, because `risk` is what triggers the human gate before a merge (§3.6). The human token may set any of them on any task. |
| Ceiling / review limit (the human) | `board ceiling 5h 85 [--wip 4]`, or drag the handle on `/status` | `POST /projects/{p}/ceilings` | The owner's override of `board-policy.json`, stored on the board and reported as `budget.source: "board"`. Sending a window empty hands it back to the policy. **Human token only** — a ceiling an agent can raise is not a ceiling (§3.7). |
| Phase (the human) | the phase control on `/status` | `POST /projects/{p}/phase` | Moving a project between `idea`/`build`/`launch`/`live` after it exists. Human token only: the phase decides whether a prod deploy is allowed and how the merge gate reads (§3.6). |
...
| Ask | `board ask --task T-42 --default "B" --deadline 8h "Should X be A or B?"` → `{id: Q-9}` | `POST /questions` | **Returns immediately.** The board pushes a notification with a link to `/q/Q-9`. The question text is rendered as markdown on that page, so structure it. `default` and `default_answer` are both accepted — the column and every read say the latter, and writing back the name you just read used to drop the default in silence. |
...
| Dispatch | `board task progress T-42 --dispatch reviewer:sonnet [--tokens 84213] [--result "3 findings"]` | `POST /tasks/{id}/progress` | `role:model`, validated — a malformed value is refused (400). `board task show` returns the chain as `dispatches` and the sum as `cost`. `cost.complete` is false while any dispatch is still missing its token count: the sum is then a FLOOR, and the estimate-versus-actual metrics leave the task out rather than average a floor into a model's record. |
