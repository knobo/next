...
| Gate | `board gate merge T-42` → exit 0/1 + a JSON reason | `GET /tasks/T-42/gate/merge` | Checks: review `open==0`, phase policy, the merge mutex, and the grant. (`launch`/`live` can still require `human.test_result` — the last remnant of the middle stage, and it is meant to go: the agent verifies itself, the human tests what is deployed.) |
